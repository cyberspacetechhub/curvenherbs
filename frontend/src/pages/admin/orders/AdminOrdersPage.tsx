    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.875rem' }}>
        {[
          ...
          { label: 'Pending', value: orders.filter((o: Order) => o.status === 'Pending').length, color: '#f59e0b' },
          { label: 'Processing', value: orders.filter((o: Order) => o.status === 'Processing').length, color: '#8b5cf6' },
          { label: 'Delivered', value: orders.filter((o: Order) => o.status === 'Delivered').length, color: '#2E7D32' },
          { label: 'Cancelled', value: orders.filter((o: Order) => o.status === 'Cancelled').length, color: '#ef4444' },
        ].map(s => (
